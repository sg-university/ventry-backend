...
def setup_function(function):
    ...
    for inventory_control in inventory_control_mock_data:
        inventory_control_repository.create_one(InventoryControl(**inventory_control.dict()))


def teardown_function(function):
    for inventory_control in inventory_control_mock_data:
        if function.__name__ == "test__delete_one_by_id__should_delete_one_inventory_control__success" \
                and inventory_control.id == inventory_control_mock_data[0].id:
            continue
        inventory_control_repository.delete_one_by_id(inventory_control.id)
...
def test__read_all__should_return_all_inventory_controls__success():
...
def test__read_one_by_id__should_return_one_inventory_control__success():
    response = test_client.get(
...
def test__create_one__should_create_one_inventory_control__success():
    inventory_control_create: InventoryControlCreate = InventoryControlCreate(
...
def test__patch_one_by_id__should_patch_one_inventory_control__success():
    inventory_control_patch: InventoryControlPatch = InventoryControlPatch(
...
def test__delete_one_by_id__should_delete_one_inventory_control__success():
