...
class ItemFileMapRepository:
    def __init__(self):
        self.datastore_utility: DataStoreUtility = DataStoreUtility()

    async def read_all(self) -> List[ItemFileMap]:
    ...
    async def read_one_by_id(self, id: UUID) -> ItemFileMap:
        async with await self.datastore_utility.create_session() as session:
    ...
    async def create_one(self, entity: ItemFileMap) -> ItemFileMap:
        async with await self.datastore_utility.create_session() as session:
            try:
                session.add(entity)
    ...
    async def patch_one_by_id(self, id: UUID, entity: ItemFileMap) -> ItemFileMap:
        async with await self.datastore_utility.create_session() as session:
            try:
                statement: expression = select(ItemFileMap).where(ItemFileMap.id == id)
                result = await session.execute(statement)
    ...
    async def delete_one_by_id(self, id: UUID) -> ItemFileMap:
        async with await self.datastore_utility.create_session() as session:
            try:
                statement: expression = select(ItemFileMap).where(ItemFileMap.id == id)
                result = await session.execute(statement)
