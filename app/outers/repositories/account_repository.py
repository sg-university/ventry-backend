...
async def read_all() -> List[Account]:
...
async def read_one_by_id(id: UUID) -> Account:
    async with await datastore_utility.create_session() as session:
...
async def read_one_by_email_and_password(email: str, password: str) -> Account:
    async with await datastore_utility.create_session() as session:
...
async def read_one_by_email(email: str) -> Account:
    async with await datastore_utility.create_session() as session:
...
async def create_one(entity: Account) -> Account:
    async with await datastore_utility.create_session() as session:
        try:
            session.add(entity)
...
async def patch_one_by_id(id: UUID, entity: Account) -> Account:
    async with await datastore_utility.create_session() as session:
        try:
            statement: expression = select(Account).where(Account.id == id)
            result = await session.execute(statement)
