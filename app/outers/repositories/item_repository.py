...
async def read_one_by_id(id: UUID) -> Item:
    async with await datastore_utility.create_session() as session:
...
async def create_one(entity: Item) -> Item:
    async with await datastore_utility.create_session() as session:
        try:
            session.add(entity)
...
async def patch_one_by_id(id: UUID, entity: Item) -> Item:
    async with await datastore_utility.create_session() as session:
        try:
            statement: expression = select(Item).where(Item.id == id)
            result = await session.execute(statement)
...
async def delete_one_by_id(id: UUID) -> Item:
    async with await datastore_utility.create_session() as session:
        try:
            statement: expression = select(Item).where(Item.id == id)
            result = await session.execute(statement)
